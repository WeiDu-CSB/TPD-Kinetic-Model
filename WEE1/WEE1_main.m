...
alpha=1;               % ternary complex cooperativit
...
VHL_fc_fit=zeros(length(WEE1_VHL_fc),length(kpr_series));
...
residual=(VHL_fc_fit-WEE1_VHL_fc).^2; 
[min_value VHL_min_index]=min(residual');
VHL_kpr_fit=kpr_series(VHL_min_index);


CRBN_fc_fit=zeros(length(WEE1_CRBN_fc),length(kpr_series));
...
residual=(CRBN_fc_fit-WEE1_CRBN_fc).^2;
...
csvwrite('WEE1_out.csv',WEE1_out);
...
IC50=zeros(length(KD_series),length(kpr_series));
IMAX=zeros(length(KD_series),length(kpr_series));
...
DHOOK=zeros(length(KD_series),length(kpr_series));
IHOOK=zeros(length(KD_series),length(kpr_series));
